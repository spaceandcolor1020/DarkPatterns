...
const Report: React.FC<ReportProps> = ({ report, resetToHome }) => {
  ...
  const { audit_results = [], global_rec = '', ethics = {} as any, transparency = {} as any, verified_citations = [], remediation_plans = [], google_pair_audit = { compliance_score: 0, principles: [] } } = report;

  // AI Presence Detector: Check for explicit signals or a confidence score above threshold
  const hasAiDetected = (transparency.signals || []).length > 0 || (transparency.ai_presence_confidence || 0) >= 10;
