...
const RESPONSE_SCHEMA = {
  ...
  properties: {
    ...
    dashboard: {
      ...
      properties: {
        ...
        transparency_label: { type: Type.STRING, enum: ['GLASS BOX', 'GREY BOX', 'BLACK BOX'] }
      },
  ...
  required: ['scan_meta', 'dashboard', 'audit_results', 'global_rec', 'ethics', 'transparency', 'verified_citations', 'remediation_plans', 'google_pair_audit']
};

export const analyzeWithGemini = async (base64Image: string, domText: string): Promise<AuditReport> => {
  // Always initialize fresh to pick up the latest platform-provided key
  ...
  try {
    ...
    // Handle the specific error per guidelines to prompt key reset
